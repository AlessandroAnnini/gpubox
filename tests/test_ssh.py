...
def test_run_ssh_publickey_hint_is_generic_off_runpod(
    ...
    def fake_run(*_a, **_k):
        return SimpleNamespace(returncode=255, stderr="Permission denied (publickey).", stdout="")

    monkeypatch.setattr("gpubox._ssh.subprocess.run", fake_run)
    with pytest.raises(AuthError, match="provider account") as exc:
        run_ssh(key, "host.example", 22, "root", "true", provider="lambda")
    assert "RUNPOD_SSH_KEY" not in str(exc.value)
