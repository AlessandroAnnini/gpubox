...
from gpubox.adapters.runpod import RunPodCloud, encode_sku, ensure_ssh_key, parse_sku, ssh_from_pod
...
def test_runpod_ensure_ssh_key_appends(tmp_path: Path) -> None:
    ...
    pub.write_text("ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAItestkey gpubox-test\n")
    ...
    def fake_graphql(query: str, variables: dict | None = None) -> dict:
        ...
        if "myself" in query:
            return {"myself": {"pubKey": "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIoldkey old"}}
        return {"updateUserSettings": {"id": "user-1"}}

    cloud._graphql = fake_graphql  # type: ignore[method-assign]
    line = ensure_ssh_key(cloud)
    assert "Itestkey" in line
    assert len(calls) == 2
    assert "updateUserSettings" in calls[1][0]
    merged = (calls[1][1] or {})["input"]["pubKey"]
    assert "Ioldkey" in merged
    assert "Itestkey" in merged


def test_runpod_ensure_ssh_key_skips_duplicate(tmp_path: Path) -> None:
    private = tmp_path / "id_ed25519"
    private.write_text("dummy-private")
    pub = tmp_path / "id_ed25519.pub"
    pub.write_text("ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAItestkey gpubox-test\n")
    ...
    def fake_graphql(query: str, variables: dict | None = None) -> dict:
        del variables
        calls.append(query)
        return {"myself": {"pubKey": "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAItestkey other-comment"}}

    cloud._graphql = fake_graphql  # type: ignore[method-assign]
    ensure_ssh_key(cloud)
    assert len(calls) == 1
    assert "updateUserSettings" not in calls[0]


def test_ensure_ssh_key_rejects_non_runpod() -> None:
    with pytest.raises(AuthError, match="RunPod-only"):
        ensure_ssh_key(FakeCloud())
